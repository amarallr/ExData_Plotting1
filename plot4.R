...
plot4 <- function() {
        ...
        ## Plot 4.2
        with(subset(dsHPC, Voltage != '?'), plot(dateTime, as.numeric(Voltage), type ="l", xlab="datetime", ylab="Voltage", col='black'))
        ...
        ## Plot 4.4
        with(subset(dsHPC, Global_reactive_power != '?'), plot(dateTime, as.numeric(Global_reactive_power), type ="l", xlab="datetime", ylab="Global_reactive_power", col='black'))
